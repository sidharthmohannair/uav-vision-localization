...
class PositionError:
    ...
    def visualize_errors(self, satellite_image):
        """
        Visualize errors on the satellite image, separated by detector
        """
        # Group errors by detector
        detector_groups = {}
        for error_data in self.errors:
            detector = error_data['detector']
            if detector not in detector_groups:
                detector_groups[detector] = []
            detector_groups[detector].append(error_data)
            
        # Create a separate visualization for each detector
        for detector, errors in detector_groups.items():
            ...
            # Plot positions and errors for this detector
            ...
            plt.imshow(vis_img)
            plt.title(f'Position Estimation Errors - {detector.upper()}')
            plt.legend()
            plt.grid(True)
            plt.savefig(f'report/position_errors_{detector}.png')
            plt.close()
    ...
    def _create_detector(self):
        if self.detector_name == "sift":
            return cv2.SIFT_create(
                nfeatures=5000,     # More features
                nOctaveLayers=8,     # More scale space
                contrastThreshold=0.02, # More sensitive
                edgeThreshold=20,   # Better edge detection
                sigma=1.6   # Gaussian blur
            )
...
class PracticalDroneEvaluator:
    def __init__(self, satellite_path: str, drone_images: List[str], drone_position: Tuple[int, int] = None):
        """
        Initialize evaluator with satellite image, drone images and optional position
        Args:
            satellite_path: Path to satellite image
            drone_images: List of drone image paths
            drone_position: Optional tuple of (x, y) coordinates. If None, will ask for interactive selection
        """
        ...
        self.position_error = PositionError()
        
        # Store drone position
        self.drone_position = drone_position
        if self.drone_position is None:
            print("Please click on the actual drone position in the satellite image:")
            self.drone_position = self.set_position_interactively(self.satellite_image)
            if self.drone_position is None:
                raise ValueError("No position was selected")
            print(f"Using position: {self.drone_position}")
    ...
    def save_visualization(self, test_case: TestCase, detector: str, 
        ...
        try:
            ...
            output_dir = f"results/{detector}/{test_case.name}"
            os.makedirs(output_dir, exist_ok=True)

            # Draw matches with thicker lines
            if len(matches) > 0:
                # Create match mask for top 50 matches
                matchesMask = [1 if i < 50 else 0 for i in range(len(matches))]
                
                # Draw parameters
                draw_params = dict(
                    matchColor=(0, 255, 0),      # Green color for matches
                    singlePointColor=(255, 0, 0), # Red color for keypoints
                    matchesMask=matchesMask,      # Mask for displaying only top matches
                    flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS
                )

                # Draw matches using updated parameters
                ...
                plt.title(f"Matches for {detector.upper()} - {test_case.name}\n"
                         f"({len(matches)} matches found)", fontsize=14)
                plt.imshow(matched_img)
                plt.axis('off')
                plt.savefig(f"{output_dir}/matches.png", 
                         bbox_inches='tight', dpi=150)
                plt.close()

            # Draw position with larger marker
            result_img = cv2.cvtColor(test_case.satellite_image, cv2.COLOR_GRAY2BGR)
            
            # Draw actual position (green)
            cv2.circle(result_img, 
                    (int(test_case.actual_position[0]), int(test_case.actual_position[1])), 
                    15, (0, 255, 0), -1)
            
            # Draw estimated position (red)
            cv2.circle(result_img, 
            ...
            # Add legend/labels
            legend_img = cv2.putText(result_img.copy(), 'Actual Position', 
                                   (30, 30), cv2.FONT_HERSHEY_SIMPLEX, 
                                   1, (0, 255, 0), 2)
            legend_img = cv2.putText(legend_img, 'Estimated Position', 
                                   (30, 60), cv2.FONT_HERSHEY_SIMPLEX, 
                                   1, (0, 0, 255), 2)

            plt.figure(figsize=(15, 15), dpi=150)
            plt.title(f"Position Comparison - {detector.upper()} - {test_case.name}", 
                    fontsize=14)
        ...
        for image_path in self.drone_images:
            ...
            for scenario, config in test_scenarios.items():
                ...
                test_cases.append(TestCase(
                    ...
                    actual_position=config["position"]
                ))
...
if __name__ == "__main__":
    try:
        ...
        DRONE_POSITION = (3645, 3542)  # Can be easily changed here

        # Initialize evaluator with your images and position
        drone_images = [
            "drone_image.jpg",
            "drone_image2.jpg",
            "drone_image3.jpg"
        ...
        evaluator = PracticalDroneEvaluator(
            ...
            drone_position=DRONE_POSITION  # Pass the position here
        )
