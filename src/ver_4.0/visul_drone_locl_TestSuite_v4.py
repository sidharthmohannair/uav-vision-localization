    ...
    def save_visualization(self, test_case: TestCase, detector: str, 
        ...
        try:
            ...
            if len(matches) > 0:
                ...
                plt.figure(figsize=(20, 10), dpi=150)  # Increased size and DPI
            ...
            cv2.circle(result_img, 
                    ...
                    15, (0, 255, 0), -1)  # Increased size
            
            # Draw estimated position (red)
            cv2.circle(result_img, 
                    (int(position[0]), int(position[1])), 
                    15, (0, 0, 255), -1)  # Increased size
            
            # Draw line between actual and estimated positions
            cv2.line(result_img, 
                    (int(test_case.actual_position[0]), int(test_case.actual_position[1])),
                    (int(position[0]), int(position[1])),
                    (255, 255, 0), 3)  # Increased thickness

            plt.figure(figsize=(15, 15), dpi=150)  # Increased size and DPI
            plt.title(f"Position Comparison - {detector} - {test_case.name}", fontsize=14)
